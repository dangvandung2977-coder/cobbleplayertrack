...
import type {
  RebuildSuggestion,
  TeamRebuildSuggestions as TeamRebuildSuggestionsValue,
} from "@/lib/team-analysis";

type TeamRebuildSuggestionsProps = {
  suggestions: TeamRebuildSuggestionsValue;
};

const SUGGESTION_STYLES: Record<RebuildSuggestion["severity"], string> = {
...
export function TeamRebuildSuggestions({
  suggestions,
}: TeamRebuildSuggestionsProps) {
  const { t } = useLanguage();
    ...
    <section className="border border-[#4d5736]/70 bg-[#15180f]/85 p-4">
      <div className="border-b border-[#4d5736]/70 pb-3">
        <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-[#d7ae45]">
          {t("analyze.rebuildSection")}
        </p>
        <h3 className="mt-1 text-xl font-black text-[#f4ead2]">
          {t("analyze.rebuildSectionSubtitle")}
        </h3>
      </div>

      <div className="mt-4 grid gap-4 2xl:grid-cols-2">
        <SuggestionColumn
          title={t("analyze.singlesRebuild")}
          suggestions={suggestions.singles}
          fallback={t("analyze.noSinglesRebuild")}
        />
        <SuggestionColumn
          title={t("analyze.doublesRebuild")}
          suggestions={suggestions.doubles}
          fallback={t("analyze.noDoublesRebuild")}
        />
      </div>
    </section>
  );
        ...
        <div className="mt-4 space-y-3">
          {suggestions.map((suggestion) => {
            ...
            const whyText = suggestion.translationKey
              ? t(`${suggestion.translationKey}.reason`, suggestion.translationValues)
              : suggestion.why;
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-[0.14em] opacity-80">
                    {t(`rebuild.categories.${suggestion.category}`)}
                  </span>
                </div>
